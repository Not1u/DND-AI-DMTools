...
for(const resource of ['modules','map-images'])try{const modules=await fs.readdir(path.join(folder,'library','data',resource));if(modules.length)throw new Error('发布目录含用户模组或地图缓存；请换用干净暂存目录。')}catch(e){if(e.code!=='ENOENT')throw e}
