...
try{
 await api('settings.set',{baseUrl:existing.baseUrl,model:existing.model,apiKey:existing.apiKey,maxSteps:6});
 ...
 const gear=await api('ai.chat',{message:'隔离测试剧情已明确：我获得一套普通皮甲和一个普通盾牌，并确认穿戴持用。先读角色卡，只补缺失物品，然后用 character.update 或 dm.action(action=character) 将皮甲 key=leather、盾牌 key=shield 写入并装备到 armor/shield；changeId 使用 live-leather、live-shield。不要只口头描述，不要发起投骰。'});
 const synced=(await api('party.sheet',{id:pc.id})).sheet;
 if(!gear.ok||synced.equipment.armor?.key!=='leather'||synced.equipment.shield?.key!=='shield')throw Error('AI装备同步失败：'+(gear.error||'未写入装备槽'));
 const mapped=(await api('map.get')).tokens.find(t=>t.id===pc.id);if(mapped.ac!==synced.ac)throw Error('AI装备与地图AC不同步');
 ...
 const battleRequest=(await api('session.pending')).request;if(!action.ok||battleRequest?.battleAbility!=='falseLife')throw Error('本地战斗动作请求未建立：'+JSON.stringify({error:action.error,reply:action.reply,steps:action.steps?.map(s=>({name:s.name,ok:s.ok,summary:s.summary})),request:battleRequest}));
 const result={equipmentSynced:true,ac:synced.ac,gearTools:gear.steps?.map(s=>s.name),battleAbility:battleRequest.battleAbility,battleTools:action.steps?.map(s=>s.name),ok:true,model:existing.model,diceCount:pending.count,firstTools:first.steps?.map(s=>s.name),resumeTools:resumed.steps?.map(s=>s.name),rangeTools:targeted.steps?.map(s=>s.name),requestKind:state.request.kind};await fs.writeFile(path.join(home,'result.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result));
