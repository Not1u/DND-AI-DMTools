> 当前桌面版 **0.6.0**（文字生成 + PDF 原图地图）：完整解压 ZIP 后使用外层 SoloTRPG.exe；保留同级 runtime 和 library。文件存放、更新及模组筹备流程见 [桌面说明](docs/DESKTOP.md)。下方旧版单文件说明仅供历史参考。
...
Git 仓库不包含 exe 和构建缓存；新 clone 请使用 Node 启动或自行构建。
...
git pull --ff-only                         # 从 origin 获取更新（先整理本地修改）
```
...
当前目录直接对应 `Not1u/DND-AI-DMTools` 的 `main` 分支。修改、验证、提交和推送都在此进行，不再通过旧仓库同步生成。

- 代码源自旧 DSH 插件工程，保留共享引擎的装载方式；本仓库不含旧工程的 `core/`、`adapters/` 和 `schema/`。
- `HANDOVER.md` 和 `README.md.repo` 留作历史参考；默认同步脚本可能覆盖当前代码，不用于日常更新。
- 引擎与插件按 mtime 热重载；UI 和 `app/` 外部文件刷新即可生效。服务端入口或监听配置修改需重启。
- `data/ai.json`（密钥与对话）已排除在 Git 之外。角色、地图等作为初始快照入库，后续不要混入无关存档变化。
