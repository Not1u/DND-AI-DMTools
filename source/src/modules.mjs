...
export async function extractModule(filename, data) {
  ...
  if (!chunks.length || !chunks.some(c => /[\p{L}\p{N}]/u.test(c.text))) throw new Error('没有提取到文字。扫描版 PDF 请先进行 OCR 文字识别，再上传。');
  ...
  return {
    ...
    async import(filename, data) {
      ...
      try {
        ...
        if (existing) return { ok: true, duplicate: true, name: existing.name, count: existing.entries.length };
        const chunks = await extractModule(filename, data);
        const name = path.basename(filename, path.extname(filename)).replace(/[\\/]/g, '-') || '未命名模组';
        const record = { id, name, filename, bytes: data.length, importedAt: new Date().toISOString(), entries: chunks.map((chunk, i) => ({ ...chunk, id: 'upload:' + id + ':' + i, book: '模组', file: '模组/' + name + '/' + (i + 1), source: 'upload' })) };
        ...
        cache = undefined;
        return { ok: true, name, count: chunks.length, chars: chunks.reduce((n, c) => n + c.text.length, 0) };
