...
角色会不会法术以 scene.get/party.spells 为准（fogCloud=云雾术）。未明确数值先查规则/模组；不伪造攻击加值、AC、奖励或余额。已执行的无效动作不扣资源。自由模式不请求移动。
...
export function toolsForMode(tools,mode){
 ...
 if(mode==='combat')return tools.filter(t=>/^(battle_|scene_get$|party_(spells|sheet|list)$|dm_action$|dice_|roll_dice$|interaction_request$|effect_|combat_(get|end|conditions|damage|economy)$|map_(get|los|area|token_update)$|rules_|mod_|narration_say$|campaign_(get|resolve|progression_get)$)/.test(t.function.name));
